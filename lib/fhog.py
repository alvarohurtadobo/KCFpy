...
def getFeatureMaps(image, k, mapp):
	...
	# IMPORTANT NOTE Added next normalization to even nombers for size X and Y, to test for possible side effects
	mapp['sizeX'] = 2*int(sizeX/2)
	mapp['sizeY'] = 2*int(sizeY/2)
	mapp['numFeatures'] = p
	zeros_size = int(mapp['sizeX']*mapp['sizeY']*mapp['numFeatures'])
	mapp['map'] = np.zeros(zeros_size, np.float32)
...
def normalizeAndTruncate(mapp, alfa):
	...
	sizeX, sizeY = int(sizeX-2), int(sizeY-2)
